...
srcDir='/scratch2/NCEPDEV/ovp/Samira.Ardani/VPPPG/Global_RTOFS/EMC_ocean-verification/satellite'
refDir=srcDir+'/fix'
...
def get_model(vDate,platform,param):
    ...
    dataDir='/scratch2/NCEPDEV/ovp/Samira.Ardani/Global/archive'
    refDir='/scratch2/NCEPDEV/ovp/Samira.Ardani/VPPPG/Global_RTOFS/EMC_ocean-verification/satellite/fix'
...
def get_ice(vDate=None):
    ...
    if vDate==None:
        files=glob('/scratch2/NCEPDEV/ovp/Samira.Ardani/OSTIA/OSTIA*.nc')
        files.sort()
        icefile=files[-1]
    else:
        icefile='/scratch2/NCEPDEV/ovp/Samira.Ardani/OSTIA/OSTIA-UKMO-L4-GLOB-v2.0_'+vDate.strftime('%Y%m%d')+'.nc'
...
def get_ghrsst(vDate=None):
    ...
    """    
    if vDate==None:
        files=glob('/scratch2/NCEPDEV/ovp/Samira.Ardani/GHRSST/GHRSST*.nc')
        files.sort()
        url=files[-1]    
    else:
        url='/scratch2/NCEPDEV/ovp/Samira.Ardani/GHRSST/GHRSST-OSPO-L4-GLOB_'+vDate.strftime('%Y%m%d')+'.nc'
...
def get_smos(vDate=None):
    ...
    """
    if vDate==None:
        files=glob('/scratch2/NCEPDEV/ovp/Samira.Ardani/SMOS/SMOS*.nc')
        files.sort()
        url=files[-1]    
    else:
        url='/scratch2/NCEPDEV/ovp/Samira.Ardani/SMOS/SMOS-L3-GLOB_'+vDate.strftime('%Y%m%d')+'.nc'
...
def get_smap(vDate=None):
    ...
    """    
    if vDate==None:
        files=glob('/scratch2/NCEPDEV/ovp/Samira.Ardani/SMAP/SMAP*.nc')
        files.sort()
        url=files[-1]    
    else:
        url='/scratch2/NCEPDEV/ovp/Samira.Ardani/SMAP/SMAP-L3-GLOB_'+vDate.strftime('%Y%m%d')+'.nc'
...
def get_aviso(vDate=None):
    ...
    if vDate==None:
        files=glob('/scratch2/NCEPDEV/ovp/Samira.Ardani/AVISO/nrt_*.nc')
        files.sort()
        filename=files[-1]    
    else:
        filename='/scratch2/NCEPDEV/ovp/Samira.Ardani/AVISO/nrt_global_allsat_phy_l4_'+vDate.strftime('%Y%m%d')+'.nc'
...
def get_woa(vDate,param):
    ...
    """    
    climoDir='/scratch2/NCEPDEV/ovp/Samira.Ardani/Global/climo/WOA'
...
def get_hycom_climo(vDate):
    ...
    the date range to create a weighted average of the straddling month fields.
    """
    climoDir='/scratch2/NCEPDEV/ovp/Samira.Ardani/Global/climo/HYCOM'
    vDate=pd.Timestamp(vDate) # just in case
    if vDate.day==15:  # even for Feb, just because
...
if __name__ == '__main__':
    ...
    imageDir='/scratch2/NCEPDEV/stmp1/Samira.Ardani/images/class-4/satellite/'+vDate
