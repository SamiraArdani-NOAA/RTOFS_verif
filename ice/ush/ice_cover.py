...
def calc_stats(obs,model,area):
    ...
    # weighted std dev -- compressed to remove masked values
    # as a precaution, match the obs mask to the area weights
    area2=area.copy()
    obs2=obs.copy()
    model2=model.copy()
    area2.mask=np.ma.mask_or(area.mask,obs.mask)
    obs2.mask=np.ma.mask_or(area.mask,obs.mask)
    model2.mask=np.ma.mask_or(area.mask,model.mask)

    obs_sdev=np.sqrt(np.cov(obs2.compressed(),aweights=area2.compressed()))
    model_sdev=np.sqrt(np.cov(model2.compressed(), aweights=area2.compressed()))
...
def process_one_day(theDate,hemisphere):
    ...
    imageDir='/scratch2/NCEPDEV/stmp1/Samira.Ardani/images/ice/'+theDate
    rtofsfile='/scratch2/NCEPDEV/ovp/Samira.Ardani/Global/archive/'+theDate+'/rtofs_glo_2ds_n024_ice.nc'
    icefile='/scratch2/NCEPDEV/ovp/Samira.Ardani/OSTIA/OSTIA-UKMO-L4-GLOB-v2.0_'+theDate+'.nc'
    dbfile='/scratch2/NCEPDEV/ovp/Samira.Ardani/VPPPG/Global_RTOFS/EMC_ocean-verification/ice/fix/global_ice.db'
    ...
    if UPLOAD_TO_POLAR:
        ...
        localDir='/scratch2/NCEPDEV/stmp1/Samira.Ardani/images/ice/'+theDate
        archDir='/scratch2/NCEPDEV/ovp/Samira.Ardani/Class-4/ice/images/'+theDate
        ...
        if MAKE_MOVIE:
            os.system('/scratch2/NCEPDEV/ovp/Samira.Ardani/VPPPG/Global_RTOFS/EMC_ocean-verification/ice/scripts/make_ice_movies.sh '+theDate)
